...
name: aw-workspace
description: >-
  The aw-workspace CLI shipped inside repos/aw-workspace itself (bin/aw-workspace,
  on PATH) — auto-discovered commands under src/cli/commands/, the marketplace
  ...
  identity-gated API. Use whenever you're asked to add a new aw-workspace
...
`repos/aw-workspace` ships a small CLI called **`aw-workspace`**
(`bin/aw-workspace`, on `PATH` inside the image) — separate from, and much
smaller than, the monolith's `./aw` (`skills/aw/SKILL.md`). Named
`aw-workspace` on purpose, not `aw`: this repo's root also contains a
...
```bash
aw-workspace help
aw-workspace marketplace install <app>
aw-workspace marketplace install <app> --update
aw-workspace update workspace
aw-workspace update remote-host --token <central-identity-jwt>
```

`aw-workspace` works from any cwd/shell inside the container — no `./`
...
bin/
  aw-workspace                  # the real launcher, on PATH (Dockerfile ENV PATH)
src/cli/
  local_client.py               # HTTP client for THIS workspace's own API
  commands/
    help.py
    marketplace.py               # aw-workspace marketplace install <app> [--update]
    update.py                    # aw-workspace update <workspace|remote-host>
```

`bin/aw-workspace` auto-discovers every module in `src/cli/commands/` via
`pkgutil.iter_modules` — same pattern as the monolith's `src/commands/`.
Adding a command is: drop `src/cli/commands/<name>.py` with

```python
COMMAND = "mycommand"          # what the user types after aw-workspace
DESCRIPTION = "One-line help"  # shown by aw-workspace help
def run(args: list[str]) -> int: ...
```

No registration step — it becomes runnable immediately (as long as
`bin/aw-workspace` is on `PATH`, which it is inside the built image; run it
as `bin/aw-workspace <cmd>` in a bare checkout without the image's `PATH`).
...
If you add a new `aw-workspace` command that needs to call this workspace's
own API, reuse `src/cli/local_client.py`'s `request(method, path,
json_body=None)` — it already attaches the token header.

## `aw-workspace update <workspace|remote-host>`

Calls **aw-backend** (the cloud control plane), not this workspace:

- `aw-workspace update workspace` → `POST {AW_BACKEND_URL}/api/workspaces/{AW_WORKSPACE}/update`
- `aw-workspace update remote-host` → `POST {AW_BACKEND_URL}/api/workspaces/{AW_WORKSPACE}/remote-host/update`
...
```bash
aw-workspace update workspace --token <aw_id_jwt value>
# or
...
The Dockerfile prepends `/opt/aw-workspace/bin` (this repo's `bin/`) onto
`PATH`, alongside the existing `/opt/aw-workspace/.aw-workspace/bin` (F4
app-shim dir — a different, unrelated bin dir for installed apps' own
shims). Both are baked in as image-level `ENV PATH`, so every
process/shell/agent session in the container sees `aw-workspace` without
extra setup.
