...
> Read the **`aw-workspace` skill** (`native-skills/aw-workspace/SKILL.md`) for the
...
**`skills/` is generated and gitignored — never edit it.** It is the merge of
two sources, and which one you edit depends on who owns the skill:

| Source | Holds | Committed? |
|---|---|---|
| `native-skills/<name>/` | the skills this repo owns | yes, here |
| an app's `contributes.skills` | everything an installed app adds | in that app's repo |

Each app-contributed copy carries a hidden `.aw-app-id` naming its owner —
that marker is how the sync knows which entries are not its to delete, and
how you find out where a skill really lives before editing it.

`skills/` itself has to keep existing as a real directory of real copies:
Codex reads it raw, the `kb` app's `load_skill` reads it raw, and app
containers bind-mount it read-only. Per-agent mirrors are copied from it:
...
Everything below `native-skills/` is regenerated by **`aw-workspace-cli agent
sync`**, which also runs on every boot (`sync_on_boot`, after app reconcile)
— that is what populates `skills/` on a fresh deployment, where it starts
out empty.

Sync semantics: **exact mirror** — files removed from a source are deleted
downstream. Anything weaker leaves an uninstalled app's skill behind,
teaching agents to call tools that no longer exist. The one exception is the
ownership rule above: `materialize()` only ever deletes native entries, so a
sync can never wipe an installed app's skills.
...
After editing `.mcp.json`, `AGENTS.md`, or anything under `native-skills/`:
...
- If you create a new convention, document it in
  `native-skills/aw-workspace/SKILL.md` so the next agent finds it.
