...
def list_skills(source: Path | None = None) -> list[dict]:
    """``[{name, description, path, rel_path, editable, owner}]`` for every
    ``skills/<name>/SKILL.md`` — the merged, generated tree, so this reflects
    what agents actually see (native + app-contributed + app-sourced).

    ``editable`` is true only for a native entry, backed by
    ``native-skills/<name>/`` and this repo's own to create/delete/edit.
    An app-owned entry (pushed via ``contributes.skills`` or pulled from a
    registered skill source — see the module docstring) carries ``owner``,
    the app id read off its ``.aw-app-id`` marker, and is read-only here.
    """
    src = source or source_dir()
    if not src.exists():
        return []
    ...
    for entry in sorted(src.iterdir()):
        ...
        owner_marker = entry / OWNER_MARKER
        owner = None
        if owner_marker.is_file():
            try:
                owner = owner_marker.read_text(encoding="utf-8").strip() or None
            except OSError:
                owner = None
        try:
...
def create_skill(name: str, description: str = "") -> Path:
    """Scaffold a new NATIVE skill at ``native-skills/<name>/SKILL.md``.

    Writes to ``native-skills/``, never to ``skills/`` directly — ``skills/``
    is regenerated by :func:`materialize` on every sync, and an entry written
    straight there with no ``native-skills/`` backing would be deleted as
    stale on the very next sync. Raises ``ValueError`` on an invalid name or
    a collision with an existing native or app-owned entry.
    """
    if not name or not all(c.isalnum() or c == "-" for c in name) or name.startswith("-"):
        raise ValueError(
            f"invalid skill name {name!r} — use lowercase alphanumerics and hyphens"
        )
    native = native_source_dir()
    skill_dir = native / name
    if skill_dir.exists():
        raise ValueError(f"skill {name!r} already exists at {skill_dir}")
    if (source_dir() / name).exists():
        raise ValueError(f"skill {name!r} is already provided by an installed app")

    skill_dir.mkdir(parents=True, exist_ok=False)
    skill_md = skill_dir / "SKILL.md"
    desc_line = description.strip().replace("\n", " ")
    skill_md.write_text(
        "---\n"
        f"name: {name}\n"
...
def delete_skill(name: str) -> bool:
    """Remove the NATIVE skill directory ``native-skills/<name>``.

    Returns ``True`` if deleted, ``False`` if no such native skill exists.
    Raises ``ValueError`` for an invalid name or an entry owned by an
    installed app — that one is the apps framework's to remove (on
    uninstall), not this editor's.
    """
    ...
    if not skill_dir.exists():
        merged = source_dir() / name
        if merged.exists() and (merged / OWNER_MARKER).is_file():
            raise ValueError(
                f"skill {name!r} is provided by an installed app; not deletable here"
            )
        return False
